...
CBigNum fdivision(const CBigNum& a, const T& b) {
    ...
    CBigNum tmp = num2;
    while(divMod(tmp, 2) == 0) {
        tmp /= 2;
        unrepeat_cnt++;
    }
    while(divMod(tmp, 5) == 0) {
        tmp /= 5;
        unrepeat_cnt++;
    }
    // Unrepeat
    ...
    // Do Exist a Repeat Period
        ...
        CBigNum tens(10);
        while(tens % tmp != 1) {
            if(tens.getInts().size() >= max((long long)(1), (long long)(a.getResFracBits()/10))) {
                return positive ? num1/num2 : -num1/num2;
            }
            tens <<= 1;
        }
        if(tens.getInts().size() == a.getResFracBits()) {
            return positive ? num1/num2 : -num1/num2;
        }
        ...
        while(repeats.size() < tens.getInts().size()-1) {
            if (num1 < num2) {
                ...
                if(res.first.getFracs().size()+1 > unrepeat_cnt+1)  {
        ...
        long long pre_length = (long long)(res.first.getFracs().size());
        for(long long i = 0; i <= a.getResFracBits()- pre_length; i++) {
            res.first.pushFrac(repeats[i%repeats.size()]);
        }
        res.first.round(a.getResFracBits());
...
// ln
CBigNum ln(const CBigNum& a, bool fd = true, bool naive = false);
template <typename T>
CBigNum ln(const T& value, bool fd = true, bool naive = false) {
    CBigNum num = any2BigNum(value);
    return ln(num, fd, naive);
}
CBigNum naive_ln(const CBigNum& a, bool fd = true, bool naive = false);
// log
template <typename T>
CBigNum log(const CBigNum& base, const T& value, bool fd = true, bool naive = false) {
    ...
    return ln(natrual, fd, naive) / ln(base, fd, naive);
}
...
* @Date: 2024/09/21
*/
long long SINCOS_N_find(const CBigNum& x, bool fd = false);
CBigNum SINTNV(const CBigNum& x, bool fd = false);
CBigNum COSTNV(const CBigNum& x, bool fd = false);
...
*/
// sin
CBigNum sin(const CBigNum& a, bool fd = false);
template <typename T>
CBigNum sin(const T& value, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return sin(num, fd);
}
// cos
CBigNum cos(const CBigNum& a, bool fd = false);
template <typename T>
CBigNum cos(const T& value, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return cos(num, fd);
}
// tan
CBigNum tan(const CBigNum& a, bool fd = false);
template <typename T>
CBigNum tan(const T& value, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return tan(num, fd);
}
// cot
CBigNum cot(const CBigNum& a, bool fd = false);
template <typename T>
CBigNum cot(const T& value, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return cot(num, fd);
}
...
// e^x
CBigNum exp(const CBigNum& a, bool fd = true, bool naive = false);
template <typename T>
CBigNum exp(const T& value, bool fd = true, bool naive = false) {
    CBigNum num = any2BigNum(value);
    return exp(num, fd, naive);
}
...
*/
// ASin Iteratoin
long long ASIN_N_find(const CBigNum& x, bool fd = false);
CBigNum ASINTNV(const CBigNum& x, bool fd = false);
// ATan Iteratoin
long long ATAN_N_find(const CBigNum& x, bool fd = false);
CBigNum ATANTNV(const CBigNum& x, bool fd = false);
// arcsin
CBigNum asin(const CBigNum& a, CBigNum PI_Given=PI, bool fd = false);
template <typename T>
CBigNum asin(const T& value, CBigNum PI_Given=PI, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return asin(num, PI_Given, fd);
}
// arccos
CBigNum acos(const CBigNum& a, CBigNum PI_Given=PI, bool fd = false);
template <typename T>
CBigNum acos(const T& value, CBigNum PI_Given=PI, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return acos(num, PI_Given, fd);
}
// arctan
CBigNum atan(const CBigNum& a, CBigNum PI_Given=PI, bool fd = false);
template <typename T>
CBigNum atan(const T& value, CBigNum PI_Given=PI, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return atan(num, PI_Given, fd);
}
// arccot
CBigNum acot(const CBigNum& a, CBigNum PI_Given=PI, bool fd = false);
template <typename T>
CBigNum acot(const T& value, CBigNum PI_Given=PI, bool fd = false) {
    CBigNum num = any2BigNum(value);
    return acot(num, PI_Given, fd);
}

// Calculate π
CBigNum PI_Calculate(long long bits, bool fd = false);
